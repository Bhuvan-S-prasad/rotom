...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
    return (
        <motion.div
            ...
            className={cn(
                "relative z-60 mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-full px-4 py-2 lg:flex bg-black/50 backdrop-blur-md border border-white/10",
        ...
        >
            {items.map((item, idx) => (
                <a
                    ...
                    className={cn(
                        "relative px-4 py-2 transition-colors text-white"
                    )}
                    key={`link-${idx}`}
                    href={item.link}
                >
